...
const PUBLIC_DIR = path.join(PROJECT_ROOT, "client", "public", "duo-videos");
const OUT_PATH = path.join(PUBLIC_DIR, "manifest.json");

const CATEGORY_DIRS = [
  { dir: "dribbling", category: "dribbling" },
  { dir: "circuit_conduite", category: "circuit_conduite" },
  { dir: "finishing", category: "finishing" },
  { dir: "first-touch", category: "first_touch" },
  { dir: "passing", category: "passing" },
];
...
function main() {
  ...
  for (const { dir, category } of CATEGORY_DIRS) {
    ...
    for (const filename of files) {
      const url = `/${encodePath(path.join("duo-videos", dir, filename))}`;
      items.push({
        category,
